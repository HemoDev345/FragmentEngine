...
namespace fm
{
    ...
    public:
        ...
        IndexBuffer(const uint32_t* indices, unsigned size, STREAM_TYPE usage)
        {
            FM_GL_CALL(glGenBuffers(1, &m_id));
            FM_GL_CALL(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_id));
            FM_GL_CALL(glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(uint32_t) * size , indices, convert_GLusage(usage)));
        }
        ...
        void set_data(const uint32_t* indices, unsigned size, STREAM_TYPE usage)
        {
            FM_GL_CALL(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_id));
            FM_GL_CALL(glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(float) * size , indices, convert_GLusage(usage)));
        }
